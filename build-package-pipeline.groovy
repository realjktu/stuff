...
node('docker') {
    ...
    stage('build-source') {
      ...
      buildSourceGbpNew('src', OS + ':' + DIST, true, 'Jenkins', 'autobuild@mirantis.com', revisionPostfix)
...
def buildSourceGbpNew(dir, image="debian:sid", snapshot=false, gitName='Jenkins', gitEmail='jenkins@dummy.org', revisionPostfix="") {
    def common = new com.mirantis.mk.Common()
        ...
        withEnv(["DEBIAN_FRONTEND=noninteractive", "DEBFULLNAME='${gitName}'", "DEBEMAIL='${gitEmail}'"]) {
            ...
            [[ "${snapshot}" == "false" ]] || (
                ...
                if [[ "`cat debian/source/format`" = *quilt* ]]; then
                    ...
                    sudo -H -E -u jenkins git tag \$NEW_UPSTREAM_VERSION_TAG \$UPSTREAM_BRANCH &&
